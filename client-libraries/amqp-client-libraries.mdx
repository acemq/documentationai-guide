...
This page documents the usage of [AMQP 1.0](/oss/next/amqp) client libraries for **RabbitMQ 4.0 or more**.
...
RabbitMQ AMQP 1.0 client libraries are safe by default, they always create [durable entities](/oss/next/queues#durability) and always publish persistent messages.
...
Libraries use the `ANONYMOUS` [SASL authentication mechanism](/oss/next/access-control#mechanisms) by default.
...
The client application take appropriate measures depending on the status (["outcome" in AMQP terms](/oss/next/amqp#outcomes)) the broker returned for the message (e.g. store the message in another place if the message has not been `accepted`).
...
Libraries translate the API calls into the [address format v2](/oss/next/amqp#address-v2).
...
Libraries provide helpers in the message creation API to define the message target, which avoids dealing with the  [address format](/oss/next/amqp#address-v2).
...
If a message is meant to go to a [stream](/oss/next/streams), it is possible to set its [filter value](/oss/next/stream-filtering) with the `x-stream-filter-value` message annotation:
...
Each term maps to a [given _outcome_](/oss/next/amqp#outcomes) in the AMQP specification.

* `accept`: the application successfully processed the message and it can be deleted from the queue (`accepted` outcome)
* `discard`: the application cannot process the message because it is invalid, the broker can drop it or [dead-letter](/oss/next/dlx) it if it is configured (`rejected` outcome)
...
__Only quorum queues__ support the [modification of message annotations](/oss/next/amqp#modified-outcome) with the `modified` outcome.
...
Libraries have out-of-the-box support for [streams](/oss/next/streams) in consumer configuration.

It is possible to set where to attach to when [consuming](/oss/next/streams#consuming) from a stream:
...
There is also support for [stream filtering](/oss/next/stream-filtering) configuration:
...
Consider also using the [native stream protocol](/oss/next/stream) with the stream client library for your preferred programming language when working with streams.
...
Here is how to create a [queue](/tutorials/amqp-concepts#queues) with [the default queue type](/oss/next/vhosts#default-queue-type):
...
The management API supports [queue arguments](/oss/next/queues#optional-arguments) explicitly:
...
Here is an example with the creation of a [quorum queue](/oss/next/quorum-queues):
...
There is also support for [exchange-to-exchange binding](/oss/next/e2e):
