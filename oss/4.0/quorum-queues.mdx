...
the `modified` outcome when using [AMQP](/oss/4.0/amqp).

For cases that would benefit from replication and repeatable reads, [streams](/oss/4.0/streams) may
be a better option than quorum queues.

Quorum queues and [streams](/oss/4.0/streams) are the two replicated data structures
...
General familiarity with [RabbitMQ clustering](/oss/4.0/clustering) would be helpful here when learning more about quorum queues.
...
When applied to queue mirroring in RabbitMQ [clusters](/oss/4.0/clustering)
...
Publishers should [use publisher confirms](/oss/4.0/publishers#data-safety) as this is how clients can interact with
the quorum queue consensus system. Publisher confirms will [only be issued](/oss/4.0/confirms#when-publishes-are-confirmed) once
...
Consumers should use [manual acknowledgements](/oss/4.0/confirms) to ensure messages that aren't
 ...
 * When data safety is not a priority (e.g. applications do not use [manual acknowledgements and publisher confirms](/oss/4.0/confirms) are not used)
 * Very long queue backlogs (5M+ messages) ([streams](/oss/4.0/stream) are likely to be a better fit)
 * Large fanouts: ([streams](/oss/4.0/stream) are likely to be a better fit)

## Features

Quorum queues share most of the fundamentals with other [queue](/oss/4.0/queues) types.
 ...
 * [Consumer acknowledgements](/oss/4.0/confirms) (except for global [QoS and prefetch](#global-qos))
...
| :-------- | :------- | ------ |
| [Non-durable queues](/oss/4.0/queues) | yes | no |
| Message replication | no | yes |
| [Exclusivity](/oss/4.0/queues) | yes | no |
| Per message persistence | per message | always |
| Membership changes | no | [semi-automatic](#member-reconciliation)   |
| [Message TTL (Time-To-Live)](/oss/4.0/ttl) | yes | yes |
| [Queue TTL](/oss/4.0/ttl#queue-ttl) | yes | partially (lease is not renewed on queue re-declaration) |
| [Queue length limits](/oss/4.0/maxlength) | yes | yes (except `x-overflow`: `reject-publish-dlx`) |
| Keeps messages in memory | see [Classic Queues](/oss/4.0/classic-queues#memory) | never (see [Resource Use](#resource-use))|
| [Message priority](/oss/4.0/priority) | yes | [yes](/oss/4.0/quorum-queues#priorities) |
| [Single Active Consumer](/oss/4.0/consumers#single-active-consumer) | yes | yes |
| [Consumer exclusivity](/oss/4.0/consumers#exclusivity) | yes | no (use [Single Active Consumer](/oss/4.0/consumers#single-active-consumer)) |
| [Consumer priority](/oss/4.0/consumer-priority) | yes | yes |
| [Dead letter exchanges](/oss/4.0/dlx) | yes | yes |
| Adheres to [policies](/oss/4.0/policies) | yes | yes (see [Policy support](#policy-support)) |
| Poison message handling | no | yes |
| [Server-named queues](/oss/4.0/queues#server-named-queues) | yes | no |
...
Quorum queues support both [Queue TTL](/oss/4.0/ttl#queue-ttl) and message TTL
(including [Per-Queue Message TTL in Queues](/oss/4.0/ttl#per-queue-message-ttl) and
 [Per-Message TTL in Publishers](/oss/4.0/ttl#per-message-ttl-in-publishers)).
When using any form of message TTL, the memory overhead increases by 16 bytes per message.

### Length Limit

Quorum queues has support for [queue length limits](/oss/4.0/maxlength).
...
Quorum queues support [dead lettering via dead letter exchanges](/oss/4.0/dlx) (DLXs).

Traditionally, using DLXs in a clustered environment has not been [safe](/oss/4.0/dlx#safety).
...
that enables dead lettering](/oss/4.0/dlx/) like so:
...
* Enable the `stream_queue` [feature flag](/oss/4.0/feature-flags) in case it is not enabled
...
[advanced config file](/oss/4.0/configure#advanced-config-file) if high dead-lettering throughput
...
Quorum queues support [consumer priorities](/oss/4.0/consumer-priority) and starting with 4.0,
they also support a type of message prioritisation that is quite different
from [classic queue message priorities](/oss/4.0/priority).
...
such that the message is never consumed completely and [positively acknowledged](/oss/4.0/confirms) so that it can be marked for
...
[dead-lettered](/oss/4.0/dlx) (if a DLX is configured).

For consumers with a [prefetch](/oss/4.0/confirms#channel-qos-prefetch) greater than 1, all outstanding
...
[stream](/oss/4.0/streams) for a low priority dead letter policy is a good, low resource way to ensure
...
To disable it, set `delivery-limit` to `-1` using a [policy](/oss/4.0/policies) or the `x-delivery-limit`
[queue argument](/oss/4.0/queues#optional-arguments).
...
The delivery limit can be set using the [policy](/oss/4.0/policies) key `delivery-limit`
...
Messages that are redelivered more times than the limit allows for will be either dropped (removed) or [dead-lettered](/oss/4.0/dlx).
...
To learn more about dead-lettering, please consult its [dedicated guide](/oss/4.0/dlx).
...
Classic queues can be [non-durable](/oss/4.0/queues). Quorum queues are always durable per their
assumed [use cases](#use-cases).

### Exclusivity

[Exclusive queues](/oss/4.0/queues#exclusive-queues) are tied to the lifecycle of their declaring connection.
...
Quorum queues are not meant to be used as [temporary queues](/oss/4.0/queues#temporary-queues).

### Global QoS

Quorum queues do not support global [QoS prefetch](/oss/4.0/confirms#channel-qos-prefetch)
...
Use [per-consumer QoS prefetch](/oss/4.0/consumer-prefetch),
which is the default in several popular clients.

## Usage

Quorum queues share most of the fundamentals with other [queue](/oss/4.0/queues) types.
Any AMQP 0.9.1 client library that can specify [optional queue arguments](/oss/4.0/queues#optional-arguments)
...
at queue declaration time; it cannot be set or changed using a [policy](/oss/4.0/policies).
...
one per each [cluster node](/oss/4.0/clustering).
...
If declaring using [management UI](/oss/4.0/management), queue type must be specified using
 ...
 * [Consumption](/oss/4.0/consumers) (subscription)
 * [Consumer acknowledgements](/oss/4.0/confirms) (keep [QoS Prefetch Limitations](#global-qos) in mind)
...
(but no fewer than three). This assumes a [fully formed](/oss/4.0/cluster-formation) cluster of at least three nodes.
...
[`forget_cluster_node`](/oss/4.0/cli) command will automatically attempt to remove all quorum queue
...
Several [CLI commands](/oss/4.0/cli) are provided to perform the above operations:
...
1. Setting the `queue-leader-locator` [policy](/oss/4.0/policies) key (recommended)
2. By defining the `queue_leader_locator` key in [the configuration file](/oss/4.0/configure#configuration-files) (recommended)
3. Using the `x-queue-leader-locator` [optional queue argument](/oss/4.0/queues#optional-arguments)
...
This also means that [upgrades](/oss/4.0/upgrade) do not trigger automatic membership reconciliation since nodes
...
`x-quorum-initial-group-size` [queue argument](/oss/4.0/queues#optional-arguments).
...
using the [publisher confirms](/oss/4.0/confirms) feature should not be lost as long as at
...
Quorum queues cannot provide any safety guarantees for messages that have not been [confirmed to the publisher](/oss/4.0/confirms).
...
Note that depending on the [partition handling strategy](/oss/4.0/partitions)
...
[`rabbitmqadmin` v2](/oss/4.0/management-cli) provides commands for automating migration from 3.13.x clusters
that use classic mirrored queues to quorum queues using the [Blue-Green Deployment strategy](/oss/4.0/blue-green-upgrade).
...
the [advanced](/oss/4.0/configure#advanced-config-file) config file.
...
When a client redeclares a queue, RabbitMQ nodes [perform a property equivalence checks](/oss/4.0/queues/#property-equivalence).
If some properties are not equivalent, the declaration will fail with a [channel error](/oss/4.0/channels#error-handling).
...
such as the ['exclusive' field](/oss/4.0/queues#exclusive-queues).
...
Keeping the queues short is the best way to maintain low memory usage. [Setting the maximum queue length](/oss/4.0/maxlength)
...
The segment files are then compacted over time as consumers [acknowledge deliveries](/oss/4.0/confirms).
...
Because memory is not guaranteed to be deallocated instantly by the [runtime](/oss/4.0/runtime/),
...
![Quorum Queues memory usage pattern](/oss/4.0/quorum-queue-memory-usage-pattern.png)
...
This follows the [general recommendation for storage](/oss/4.0/production-checklist#storage) for modern release series.

</Callout>

With large messages (say, 1 MiB and higher), quorum queue disk footprint can be large.
Depending on the workload — and in particular, the number of messages pending [consumer acknowledgement](/oss/4.0/consumers) —
...
2. Quorum queues depend heavily on consumers acknowledging messages in a timely manner,
   so a [reasonably low delivery acknowledgement timeout](/oss/4.0/consumers#acknowledgement-timeout) must be used
...
[reject or nack](/oss/4.0/nack) the same message with the `requeue` flag set to true
...
See [the Runtime guide](/oss/4.0/runtime#atom-usage) to learn more.
