...
RabbitMQ can use LDAP to perform [authentication and authorisation](/oss/4.0/access-control) by deferring to external LDAP
...
is included in the [Erlang/OTP](/oss/4.0/which-erlang) distribution on most
operating systems, including Windows and RPM-based Linux with Team RabbitMQ's [zero dependency Erlang package](/oss/4.0/install-rpm).
...
Please see the [Erlang compatibility guide](/oss/4.0/which-erlang) to learn more.

## Enabling the Plugin

The LDAP plugin ships with RabbitMQ. To enable it, use
[rabbitmq-plugins](/oss/4.0/man/rabbitmq-plugins.8):
```bash
 ...
 * Listing LDAP as an [authentication (authN) and/or authorization (authZ) backend](/oss/4.0/access-control)
...
In [`advanced.config` file](/oss/4.0/configure#erlang-term-config-file), the same settings would look like this:
...
Same example in the [`advanced.config` format](/oss/4.0/configure#erlang-term-config-file):
...
In the [advanced config format](/oss/4.0/configure#erlang-term-config-file):
...
Starting with Erlang 26, [TLS client peer verification](/oss/4.0/ssl#peer-verification) is enabled by default by the TLS implementation.
...
are very similar to [TLS settings elsewhere in RabbitMQ](/oss/4.0/ssl).
...
There are multiple [TLS](/oss/4.0/ssl) client options available:
...
Hostname verification should not be confused with [peer certificate chain verification](/oss/4.0/ssl#peer-verification).
...
Starting with Erlang 26, LDAP TLS clients will perform [peer certificate chain verification](/oss/4.0/ssl#peer-verification)
by default.

[Peer certificate chain verification](/oss/4.0/ssl#peer-verification) should not be confused with hostname match verification.
These settings are orthogonal and can be combined.
...
The same examples in the [`advanced.conf` format](/oss/4.0/configure):
...
[Certificate chain verification depth](/oss/4.0/ssl#peer-verification-depth) can be increased
...
The below example uses an [`advanced.config` format](/oss/4.0/configure#advanced-config-file):
...
A special [cache backend](/oss/4.0/auth-cache-backend/)
can be used in [combination](/oss/4.0/access-control#combined-backends) with other backends to significantly
...
to the [Access Control guide](/oss/4.0/access-control) to learn more.
...
[EXTERNAL authentication mechanism](/oss/4.0/authentication)).
...
Note that it uses the [classic config format](/oss/4.0/configure):
...
RabbitMQ [permission model](/oss/4.0/access-control)
...
which [tags](/oss/4.0/management#permissions) they
have.
...
so they can be defined only using the [advanced config format](/oss/4.0/configure#advanced-config-file).
...
also [topic authorisation](/oss/4.0/access-control#topic-authorisation)
...
Bringing it all together, here's a sample configuration. It uses both the [standard config](/oss/4.0/configure#config-file) and
[advanced config](/oss/4.0/configure#advanced-config-file) files together. This
...
The [standard config](/oss/4.0/configure#config-file)
...
[Advanced config](/oss/4.0/configure#advanced-config-file) is used to define LDAP queries:
...
Alternatively, you can use the [classic config format](/oss/4.0/configure#erlang-term-config-file)
...
some topics covered in the [Network Troubleshooting](/oss/4.0/troubleshooting-networking)
and [TLS Troubleshooting](/oss/4.0/troubleshooting-ssl) guides apply to LDAP.
