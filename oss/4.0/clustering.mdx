...
and more. [Cluster Formation and Peer Discovery](/oss/4.0/cluster-formation) is a closely related guide
that focuses on peer discovery and cluster formation automation-related topics. For queue contents
(message) replication, see the [Quorum Queues](/oss/4.0/quorum-queues) guide.
...
RabbitMQ Stream Protocol clients [can connect to multiple nodes at once](/oss/4.0/stream-connections).
 ...
 * Declaratively by listing cluster nodes in [config file](/oss/4.0/configure)
...
These mechanisms are covered in more details in the [Cluster Formation guide](/oss/4.0/cluster-formation).
...
[CLI tools](/oss/4.0/cli) also identify and address nodes using node names.

When a node starts up, it checks whether it has been assigned a node name. This is done
via the `RABBITMQ_NODENAME` [environment variable](/oss/4.0/configure#supported-environment-variables).
...
For server nodes this is done by setting the `RABBITMQ_USE_LONGNAME` [environment variable](/oss/4.0/configure#supported-environment-variables)
to `true`.
...
To use FQDNs, see `RABBITMQ_USE_LONGNAME` in the [Configuration guide](/oss/4.0/configure#supported-environment-variables).
See [Node Names](#node-names) above.

## Ports That Must Be Opened for Clustering and Replication

RabbitMQ nodes [bind to ports](/oss/4.0/networking#ports) (open server TCP sockets) in order to accept client and CLI tool connections.
 ...
 * 4369: [epmd](/oss/4.0/networking#epmd), a helper discovery daemon used by RabbitMQ nodes and CLI tools
 * 6000 through 6500: used by [RabbitMQ Stream](/oss/4.0/stream) replication
 * 25672: used for inter-node and CLI tools communication (Erlang distribution server port)
   and is allocated from a dynamic range (limited to a single port by default,
   computed as AMQP port + 20000). Unless external connections on these ports are really necessary (e.g.
   the cluster uses [federation](/oss/4.0/federation) or CLI tools are used on machines outside the subnet),
   these ports should not be publicly exposed. See [networking guide](/oss/4.0/networking) for details.
 * 35672-35682: used by CLI tools (Erlang distribution client ports) for communication with nodes
   and is allocated from a dynamic range (computed as server distribution port + 10000 through
   server distribution port + 10010).

It is possible to [configure RabbitMQ](/oss/4.0/configure)
to use different ports and specific network interfaces.
See [RabbitMQ Networking guide](/oss/4.0/networking) to learn more.
...
the [Quorum Queues](/oss/4.0/quorum-queues) guide.
...
This topic becomes more nuanced when [quorum queues](/oss/4.0/quorum-queues) and plugins
are taken into consideration but for most intents and purposes,
all cluster nodes should be considered equal.

Many [CLI tool](/oss/4.0/cli) operations can be executed against any node.
An [HTTP API](/oss/4.0/management) client can target any cluster node.

Individual plugins can designate (elect)
certain nodes to be "special" for a period of time. For example, [federation links](/oss/4.0/federation)
are colocated on a particular cluster node. Should that node fail, the links will
be restarted on a different node.

In older (long maintained) versions, [RabbitMQ management plugin](/oss/4.0/management) used
...
this strategy is not really viable in a [clustered environment](/oss/4.0/clustering).
...
When a node starts, it will [log](/oss/4.0/logging) the home directory location of its effective user:
...
Unless any [server directories](/oss/4.0/relocate) were overridden, that's the directory where
...
See the [CLI Tools guide](/oss/4.0/cli) for more information.
...
The commands are covered in the [Networking guide](/oss/4.0/networking#dns-verify-resolution).
...
Because several features (e.g. [quorum queues](/oss/4.0/quorum-queues), [client tracking in MQTT](/oss/4.0/mqtt))
...
The [Quorum Queues guide](/oss/4.0/quorum-queues) covers this topic in more detail.
...
[quorum queue leader](/oss/4.0/quorum-queues) transparently to clients.
...
With [quorum queues](/oss/4.0/quorum-queues) and [streams](/oss/4.0/streams), clients will only be able to perform
...
See the [stream client connection guide](/oss/4.0/stream-connections) to learn more.
...
1. [the configuration file](/oss/4.0/configure#configuration-files), by setting `queue_leader_locator`
1. [optional queue argument](/oss/4.0/queues#optional-arguments), by setting the `x-queue-leader-locator` ([not recommended](/oss/4.0/parameters))
...
if a different node is picked. For example, [for short-lived queues](/oss/4.0/queues#temporary-queues), `client-local` is probably
a better choice. [Exclusive queues](/oss/4.0/queues#exclusive-queues) are always declared locally.
...
Operators need to be able to inspect and [monitor](/oss/4.0/monitoring) such resources
across all cluster nodes.

RabbitMQ [CLI tools](/oss/4.0/cli) such as `rabbitmq-diagnostics` and `rabbitmqctl`
...
[Management UI](/oss/4.0/management) works similarly: a node that has to respond to an HTTP API request
...
[Quorum queue](/oss/4.0/quorum-queues) allows queue contents to be replicated
across multiple cluster nodes with parallel replication and a predictable [leader election](/oss/4.0/quorum-queues#leader-election)
and [data safety](/oss/4.0/quorum-queues#data-safety) behavior as long as a majority of replicas are online.

Non-replicated classic queues can also be used in clusters. Their behaviour in case of node failure
depends on [queue durability](/oss/4.0/queues#durability).

RabbitMQ clustering has several modes of dealing with [network partitions](/oss/4.0/partitions),
...
The [Shovel](/oss/4.0/shovel) or
[Federation](/oss/4.0/federation)
plugins are better solutions for connecting brokers across a
WAN. Note that [Shovel and Federation are not equivalent to clustering](/oss/4.0/distributed).
...
Node that responds to an [HTTP API](/oss/4.0/management) request contacts its peers
to retrieve their data and then produces an aggregated result.

In older (long unmaintained) versions [RabbitMQ management plugin](/oss/4.0/management) used
a dedicated node for stats collection and aggregation.
...
[more automation-friendly](/oss/4.0/cluster-formation) cluster formation
options are used.
...
For production environments, it is strongly recommended to use one of the automated [peer discovery mechanisms](/oss/4.0/cluster-formation) instead of manual cluster formation with CLI tools. Manual cluster formation (`rabbitmqctl join_cluster`) is highly prone to human error and race conditions. Manual CLI joins should be strictly reserved for local development, testing, or specific edge-case recovery scenarios.
...
newly joining members must be [reset](/oss/4.0/man/rabbitmqctl.8#reset).
...
a node must be [reset](/oss/4.0/man/rabbitmqctl.8#reset) before it can join an existing cluster.
...
using the [Blue/Green deployment strategy](/oss/4.0/blue-green-upgrade) or [backup and restore](/oss/4.0/backup)
...
During [upgrades](/oss/4.0/upgrade), sometimes the last node to stop
...
In some environments, node restarts are controlled with a designated [health check](/oss/4.0/monitoring#health-checks).
...
[Most health check](/oss/4.0/monitoring#health-checks), even relatively basic ones, implicitly assume that the node has
...
assuming they are [compatible](/oss/4.0/upgrade).

### Hostname Changes Between Restarts

A node rejoining after a node name or host name change can start as [a blank node](/oss/4.0/cluster-formation#peer-discovery-how-does-it-work)
...
In addition, if the cluster [peer discovery mechanisms](/oss/4.0/cluster-formation)
support node health checks and [forced removal of nodes](/oss/4.0/cluster-formation#node-health-checks-and-cleanup) not known to the discovery backend.
...
on said host can [connect and authenticate](/oss/4.0/cli) to the target node.
...
When a node is removed from the cluster using CLI tools, all [quorum queue](/oss/4.0/quorum-queues#member-management)
and [stream replicas](/oss/4.0/streams#member-management) on the node will be removed,
...
by some [peer discovery](/oss/4.0/cluster-formation) plugins, there are no scenarios
...
Resetting a node will delete all of its data, cluster membership information, configured [runtime parameters](/oss/4.0/parameters),
...
For nodes that fail to start this is already the case. Then [override](/oss/4.0/relocate)
...
For [quorum queue](/oss/4.0/quorum-queues) and [stream](/oss/4.0/streams) contents to be replicated to the new [re]added node,
...
cluster using the `forget_cluster_node` [rabbitmqctl](/oss/4.0/cli) command.

Alternatively `force_boot` [rabbitmqctl](/oss/4.0/cli) command can be used
...
guide](/oss/4.0/configure#supported-environment-variables), as well as `RABBITMQ_MNESIA_DIR`,
`RABBITMQ_CONFIG_FILE`, and
`RABBITMQ_LOG_BASE` in the [File and Directory Locations guide](/oss/4.0/relocate).
...
Note that if the node [listens on any ports](/oss/4.0/networking) other
...
See the section on [hostname resolution](/oss/4.0/clustering#hostname-resolution-requirement) for more information.
...
Learn more in the [section on ports](#ports) above and dedicated [RabbitMQ Networking guide](/oss/4.0/networking).

## Erlang Versions Across the Cluster

All nodes in a cluster are *highly recommended* to run the same major [version of Erlang](/oss/4.0/which-erlang): `26.2.0`
